...
def main():
    ...
    if int(get_git_status()) > 0:
        ...
        os.chdir("ServiceAppointmentPlugin")

        current_number_of_commits = get_commit_count()
        packages = [
            'Microting.eForm', 'Microting.eFormApi.BasePn', 'Microting.AppointmentBase'
        ]
        process_repository('ServiceAppointmentPlugin.csproj', packages, 'eform-service-appointment-plugin')
        ...
        if new_number_of_commits > current_number_of_commits:
            ...
            os.chdir("..")
            run_command(f"github_changelog_generator -u microting -p eform-service-appointment-plugin -t {os.getenv('CHANGELOG_GITHUB_TOKEN')}", shell=True)
